...
# Pull the Mistral Small 24B model (optimized for A100 — downloads ~14 GB)
ollama pull mistral-small:24b
```

> **Note:** If your server does not have a GPU with sufficient VRAM (24 GB+),
> you can use the smaller `mistral` model instead: `ollama pull mistral`
> and set `OLLAMA_MODEL=mistral` in `.env`.

Verify it is running:

```bash
ollama list
# Should show "mistral-small:24b" in the output
...
> **Important:** `host.docker.internal` works because `docker-compose.yml` maps it
> to the host via `extra_hosts: ["host.docker.internal:host-gateway"]`. This is
> required on Linux — without it, containers cannot reach host services via localhost.
>
> If you used the smaller `mistral` model instead of `mistral-small:24b`,
> set `OLLAMA_MODEL=mistral` here.
