...
Commands:
    create-admin     Create a superuser account (or promote existing user)
...
async def create_admin():
    """Create an admin (superuser) account directly."""
    if len(sys.argv) < 4:
        print("Usage: python -m backend.manage create-admin <email> <password>")
        return
    email = sys.argv[2]
    password = sys.argv[3]
